    ...
    if request.method == 'POST':
        if request.form.get('submission') and request.form['card_type']=='not_selected':
            ...
            if request.form['card_type']=='monster':
                ...
                query=f'''
                        SELECT monster_card.*, monster_box.card_text 
                        FROM monster_card
                        INNER JOIN monster_box ON monster_card.card_name=monster_box.card_name_box
                        WHERE {name_filter} AND {level_filter} AND {atk_filter} AND {def_filter}
                        AND {atr_filter} AND {type_filter}
                    '''
                # card_name에 대응되는 serial_number, card_number를 가져오는 query
                query_card_num='''
                                SELECT monster_card.card_name, belong_monster.serial_number,
                                belong_monster.card_number
                                FROM monster_card
                                INNER JOIN belong_monster ON monster_card.card_name=belong_monster.card_name  
                            '''
            # 마법 카드 선택
            elif request.form['card_type']=='magic':
                session['card_type']='magic'
                # 효과
                ...
                # 최종 마법 카드 query문
                query=f'''
                        SELECT magic_card.*, magic_box.card_text 
                        FROM magic_card
                        INNER JOIN magic_box on magic_card.card_name=magic_box.card_name
                        WHERE {name_filter} AND {icon_filter}
                    '''
                # card_name에 대응되는 serial_number, card_number를 가져오는 query
                query_card_num='''
                                SELECT magic_card.card_name, belong_magic.serial_number,
                                belong_magic.card_number
                                FROM magic_card
                                INNER JOIN belong_magic ON magic_card.card_name=belong_magic.card_name
                            '''
            # 함정 카드 선택
            else:
                session['card_type']='trap'
                # 효과
                if not sql_sender.icon_list:
                    icon_filter=True
                else:
                ...
                # 최종 함정 카드 query문
                query=f'''
                        SELECT trap_card.*, trap_box.card_text 
                        FROM trap_card
                        INNER JOIN trap_box on trap_card.card_name=trap_box.card_name
                        WHERE {name_filter} AND {icon_filter}
                    '''
                # card_name에 대응되는 serial_number, card_number를 가져오는 query
                query_card_num='''
                                SELECT trap_card.card_name, belong_trap.serial_number,
                                belong_trap.card_number
                                FROM trap_card
                                INNER JOIN belong_trap ON trap_card.card_name=belong_trap.card_name  
                            '''
            # apack 값을 가져오는 query
            query_pack='SELECT * FROM apack'
            # query문 출력값 확인용
            print('**************query***************')
            print(query)
            print('*'*10)
            print(query_card_num)
            print('*'*10)
            print(query_pack)
            ...
            curs.execute(query)
            #data = curs.fetchall()
            # 추후 내부값 수정위해 tuple서 list로 변환
            result.data=list(curs.fetchall())
            result.data=[list(data) for data in result.data]
            result.data_len=len(result.data)
            
            # card_name에 대응되는 serial / card_num 저장 (name을 key, num들을 value 형식으로)
            curs.execute(query_card_num)
            name_num_list=list(curs.fetchall())
            for name_num in name_num_list:
                name,serial_num,card_num=name_num[0],name_num[1],name_num[2]
                # card_number 수정 ex) 1 ㅡ> 001,  12 ㅡ> 012,  123 ㅡ> 123
                card_num='0'*(3-len(str(card_num)))+str(card_num)
                result.name_num_dict[name].append([serial_num,card_num])

            # serial_num에 대응되는 pack / release_year 저장 (serial_num에을 key, pack/release_year를 value 형식으로)
            curs.execute(query_pack)
            num_pack_list=list(curs.fetchall())
            print(len(num_pack_list))
            print(num_pack_list[0])
            print('*'*20)
            for num_pack in num_pack_list:
                serial_num,pack,release_year=num_pack[3],num_pack[0],num_pack[2]
                result.num_pack_dict[serial_num].append([pack,release_year])


            # 각 tuple에 zip써서 index 추가
            # 기존 :   ['atr1','atr2',atr3',...]
            # 변경후 : [[0], ['atr1','atr2',atr3',...]]
            # result.data[0]은 idx  result.data[1]은 원래 tuple
            # ex) result.idx_and_data[1][0] = 2번째(idx 1) tuple의 index
            #     result.idx_and_data[1][1] = 2번째 tuple의 내용 

            idx_list=[i for i in range(result.data_len)]
            result.idx_and_data=list(zip(idx_list,result.data))

            #template_data_len = len(data)
            """finally:
                conn.close()"""
            # 검색 결과들 월드컵 참가 리스트에 넣기
            worldcup.initial_participant_list=list(result.data)[:]
            #return render_template('search_result.html',template_data=data,template_data_len=len(data))
            session['order_value']=False
...
def search_result():
    print('*'*30)
    print("elif request.form.get('reorder'):")
    print(request.form.get('reorder'))
    print('*'*30)
    if request.method=='POST':
        # 상세보기 버튼 클릭
        if request.form.get('idx'):
            result.selected_idx=int(request.form.get('idx'))
            result.selected_row=result.data[result.selected_idx]
            return redirect('/card_info')
        # 월드컵 시작 버튼 클릭
        elif request.form.get('worldcup_start'):
            return redirect('/worldcup_main')
        # 이름순으로 정렬
        elif request.form.get('reorder'):
            if session['order_value'] != True:
                session['order_value'] = True
            else:
                session['order_value'] = False
            result.idx_and_data.sort(reverse=session['order_value'])
            return redirect('search_result')

        # 다시 검색(메인 페이지로) 버튼 클릭
        else:
            del session['card_type']
            session.pop('order_value')
            return redirect('/mainpage')
    
    return render_template('search_result.html', template_data=result.idx_and_data, template_data_len=result.data_len)
...
def card_info():
    if request.method=='POST':
        ...
        # 메인 페이지로
            ...
            del session['card_type']
            return redirect('/mainpage')
    selected_name=result.selected_row[0]
    pack_year_list=[]
    for serial_num,card_num in result.name_num_dict[selected_name]:
        pack_year_list.append([result.num_pack_dict[serial_num][0][0],result.num_pack_dict[serial_num][0][1]])
    nums_pack_year_list=list(zip(result.name_num_dict[selected_name],pack_year_list))
    print(nums_pack_year_list)
    return render_template('card_info.html',info=result.selected_row,
                            nums_pack_year_list=nums_pack_year_list)
...
def change():
    ...
    if request.method == 'POST':
        ...
        elif request.form.get('excommunicate'):
            print('탈퇴')
            curs = conn.cursor()
            sql = "Delete from account where ID =" + f"'{session['ID']}'"
            curs.execute(sql)
            print(sql)
...
def worldcup_main():
    print('worldcup url move complete')
    if request.method == 'POST':
        ...
        """# 참가한 카드들 join +1 (UPDATE)
        name_list=[participant[0] for participant in worldcup.participant_list]
        if session['card_type']=='monster':
            query_join=f'UPDATE monster_card SET join_count=join_count+1 WHERE name in {name_list}'
            curs = conn.cursor()
            curs.execute(query_join)"""
...
def worldcup_match():
    # 각 라운드(O강) 끝날 때마다 해당 라운드에 참가한 카드들 join / win / loseUPDATE 
    # 참가 리스트->승자 리스트, 승자 리스트, 패자 리스트->빈 리스트로 초기화
    if not worldcup.participant_list:
        winner_tuple=tuple([winner[0] for winner in worldcup.winner_list])
        loser_tuple=tuple([loser[0] for loser in worldcup.loser_list])

        query_win=f'UPDATE {session["card_type"]}_card SET win_count=win_count+1, join_count=join_count+1 WHERE card_name in {winner_tuple}'
        query_lose=f'UPDATE {session["card_type"]}_card SET lose_count=lose_count+1, join_count=join_count+1 WHERE card_name in {loser_tuple}'

        curs = conn.cursor()
        curs.execute(query_win)
        conn.commit()
        curs.execute(query_lose)
        conn.commit()
    ...
    return render_template('worldcup_match.html',
        ...
        left_num=result.name_num_dict[worldcup.participant_list[0][0]],
        right_num=result.name_num_dict[worldcup.participant_list[1][0]]
    )

@app.route("/selected_left")
def selected_left():
    worldcup.winner_list.append(worldcup.participant_list.pop(0))
    worldcup.loser_list.append(worldcup.participant_list.pop(0))
    # 결승전
...
@app.route("/winner")
def winner():
    # 결승전 우승자는 join, win, vic +1 / 패배자는 lose +1로 UPDATE
    winner=worldcup.winner_list.pop(0)
    loser=worldcup.loser_list.pop(0)
    # 나중에 f'{session['card_type']}'로 코드수 줄이기
    query_victory=f'''UPDATE {session['card_type']}_card
                        SET win_count=win_count+1, join_count=join_count+1, vic_count=vic_count+1
                        WHERE card_name="{winner[0]}"'''
    query_lose=f'''UPDATE {session['card_type']}_card
                        SET lose_count=lose_count+1, join_count=join_count+1
                        WHERE card_name="{loser[0]}"'''
    curs=conn.cursor()
    curs.execute(query_victory)
    conn.commit()
    curs=conn.cursor()
    curs.execute(query_lose)
    conn.commit()
